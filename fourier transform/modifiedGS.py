# -*- coding: utf-8 -*-
"""
Created on Fri Aug 25 10:43:14 2023
...
from fourier_transfrom import resize
...
for t in range(s):
    ...
    ampf = np.abs(iterf) # normalized training intenstiy
    angf = np.angle(iterf) # normalized training angle
    
    
    plot(ox, oy, np.square(ampf), 'Blues')
    
    camp = target * rg + ampf * bg
    cph = targetphase * rg + angf * bg
    cphase = np.exp(1j * cph)
    
    iterb = ft(oy, ox, y, x, camp * cphase, f, l)
    angb = np.angle(iterb)
    DOE = angb
    
    #------------------------Error calculation
    error = np.square(target) - np.square(ampf) # Calculate error
    E = np.sum(np.abs(error)) / np.sum(rga) # mean error for break iteration
    squaredDifferences = error**2 # error square
    meanSquaredDifferences = np.sum(squaredDifferences)/np.sum(rga) 
